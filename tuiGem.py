        ...
        if original_full_path := original_path_map.get(filename):
            for correct_path in correct_paths_list:
                try:
                    ...
                    # Store content as a list of lines from the start
...
def get_normalized_string(input_string):
    """Helper for the robust find, trims whitespace and leading comment chars."""
    trimmed = input_string.strip()
    if trimmed.startswith("#"): return trimmed[1:].strip()
    return trimmed

def find_text_robustly(editor_lines, text_to_find):
    """A semantic search that ignores whitespace and leading comment characters."""
    ...
    for i in range(len(editor_lines) - len(chunk_lines) + 1):
        if get_normalized_string(editor_lines[i]) == chunk_lines[0]:
            ...
            if is_full_match:
                return i # Return the starting line number of the match
    return -1
...
def main(stdscr, file_data):
    ...
    curses.init_pair(7, curses.COLOR_BLACK, curses.COLOR_YELLOW) # Highlight for deletion
    ...
    state = {
        ...
        "confirm_delete_line": -1, "delete_len": 0
    }
    ...
    while True:
        ...
        status_bar = "Arrows: Navigate | Enter: Select/Action | Tab: Switch | s: Save | u: Undo | q: Quit"
        if state["confirm_delete_line"] != -1:
            status_bar = f"Confirm deletion of text at line {state['confirm_delete_line'] + 1}? [y/n]"
        ...
        if key == ord('\t'): state["active_panel"] = ["files", "editor", "chunks"][(["files", "editor", "chunks"].index(state["active_panel"]) + 1) % 3]
            ...
            if active_file.get("backup_content"): active_file["content"] = active_file["backup_content"]; active_file["backup_content"] = None
        elif key == ord('s'):
            ...
            else: state["scroll_pos"][ap] = max(0, state["scroll_pos"][ap]+delta)
            ...
            if state["active_panel"] == "files":
                ...
                if selected_chunk["action"] == "Added":
                    ...
                    block = header + selected_chunk["code"].split('\n') + footer
                    active_file["content"][state["scroll_pos"]["editor"]:state["scroll_pos"]["editor"]] = block
                else: # Removed

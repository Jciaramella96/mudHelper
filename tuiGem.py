...
def main(stdscr, file_data):
    ...
    while True:
        ...
        for i, chunk in enumerate(active_file["chunks"]):
            color = curses.color_pair(3) if chunk["action"] == "Added" else curses.color_pair(4)
            chunk_lines.append( (f"Chunk {i+1}: {chunk['action']}", color, i) )
        for i in range(panel_height):
            idx = state["scroll_pos"]["chunks"] + i
            if idx >= len(chunk_lines): break
            ...
            if state["active_panel"] == "files":
                ...
                if selected_chunk["action"] == "Added":
                    ...
                    if match_line != -1:
                        ...
                        state["scroll_pos"]["editor"] = max(0, match_line - 2) # Scroll match into view

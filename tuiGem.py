...
def main(stdscr, file_data):
    ...
    while True:
        # Before any drawing, get the current size. This handles the initial case and resizes.
        screen_height, screen_width = stdscr.getmaxyx()
        ...
        if key == 17: # Ctrl+Q
            ...
            # Resize panels
            l_p.resize(screen_height - 1, l_w)
            c_p.resize(screen_height - 1, c_w)
            r_p.resize(screen_height - 1, r_w)
            
            # Move panels to their new positions
            c_p.mvwin(1, l_w)
            r_p.mvwin(1, l_w + c_w)
            
            # Clear and REFRESH the main screen before continuing
            stdscr.clear()
            stdscr.refresh() 
            continue # Continue to the next loop to redraw everything correctly
        elif key == 19: # Ctrl+S
            try:

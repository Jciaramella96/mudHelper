...
method: str          # ‘exact’, ‘hint’, ‘fuzzy’, ‘none’

# Strip and collapse all whitespace for loose comparison
...
if hint_idx >= 0 and hint_idx + len(change_lines) <= n:
    ...
    if score >= 0.95:
        return MatchResult(found=True, line_start=hint_start,
                           line_end=hint_start + len(change_lines) - 1,
                           confidence=score, method="exact")

# 2. Scan whole file for best exact match
best_idx = -1
best_score = 0.0
...
if best_score >= 0.95:
    return MatchResult(found=True, line_start=best_idx + 1,
                       line_end=best_idx + len(change_lines),
                       confidence=best_score, method="exact")

# 3. Fuzzy match within window around hint
...
if best_window_score >= confidence_threshold:
    return MatchResult(found=True, line_start=best_window_idx + 1,
                       line_end=best_window_idx + len(change_lines),
                       confidence=best_window_score, method="fuzzy")

# 4. Best match anywhere in file
...
return MatchResult(found=False, line_start=hint_start, line_end=hint_end,
                   confidence=best_score, method="none")
```

# Find the best line to insert at, starting from hint_start.

# If the hint line is empty/whitespace use it, otherwise scan downward.
...
n = len(file_lines)
idx = max(0, min(hint_start - 1, n))  # 0-based, clamped

```
# Use hint line if it's empty
if idx < n and not file_lines[idx].strip():
    return idx + 1

# Scan downward for next empty line

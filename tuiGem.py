...
def find_files_in_repo(filenames_to_find, repo_path):
    ...
    for report_filename in filenames_to_find:
        base_name_report = os.path.splitext(os.path.basename(report_filename))[0]
        if base_name_report in repo_file_map:
             found_map[report_filename] = repo_file_map[base_name_report]
    return found_map
    ...
    for original_full_path, correct_paths_list in found_files_map.items():
        for correct_path in correct_paths_list:
            try:
                ...
                lines = content.split('\n')
                # Match the report data using the original full path
...
class Editor:
    ...
    def handle_input(self, key, panel_height):
        ...
        if not self.buffer: self.buffer = [""] # Handle empty buffer
    ...
    def draw(self, window):
        ...
        for i in range(panel_height):
            ...
            if buffer_idx < total_lines:
                line_to_draw = self.buffer[buffer_idx]
...
# ---------------------------------------------------------------------
# 3. CORRECTED MAIN APPLICATION LOOP
# ---------------------------------------------------------------------

def main(stdscr, file_data):
    # --- SETUP: NON-BLOCKING INPUT ---
    stdscr.nodelay(True)  # This is the key change for non-blocking input
    stdscr.keypad(True)
    curses.curs_set(0)
    curses.start_color()
    ...
    while True:
        ...
        # If no key is pressed, getch() returns curses.ERR. Sleep and continue.
        if key == curses.ERR:
        ...
        if key == 17: # Ctrl+Q
            ...
            stdscr.clear() # Clear screen, the loop will handle redrawing everything
            continue
        elif key == 19: # Ctrl+S
            try:
        ...
        # --- DRAWING AND REFRESH LOGIC (at the end of the loop) ---
        screen_height, screen_width = stdscr.getmaxyx()
        panel_height = screen_height - 3
        
        # Create/resize windows dynamically to handle terminal resizing

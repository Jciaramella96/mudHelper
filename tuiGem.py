...
# ---------------------------------------------------------------------
# 1. CORE PARSING LOGIC (Restored and Unchanged)
# ---------------------------------------------------------------------
...
def find_files_in_repo(filenames_to_find, repo_path):
    ...
    for root, _, files in os.walk(repo_path):
        for name in files:
            if '.' in name:
                # Using os.path.splitext is a bit more robust
                base_name, _ = os.path.splitext(name)
                full_path = os.path.join(root, name)
                if base_name not in repo_file_map: repo_file_map[base_name] = []
                repo_file_map[base_name].append(full_path)

    for report_filename_base, _ in (os.path.splitext(os.path.basename(f)) for f in filenames_to_find):
        if report_filename_base in repo_file_map:
             # Find the original full path from the report to map back
            original_full_path = next((f for f in filenames_to_find if os.path.splitext(os.path.basename(f))[0] == report_filename_base), None)
            if original_full_path:
                found_map[original_full_path] = repo_file_map[report_filename_base]
...
def prepare_file_data(report_data):
    ...
    report_filenames = list(report_data.keys())
    found_files_map = find_files_in_repo(report_filenames, REPO_SEARCH_PATH)
    ...
    for original_full_path, correct_paths_list in found_files_map.items():
        for correct_path in correct_paths_list:
            try:
                ...
                # Ensure the original_full_path from the report is used to get the chunks
...
if __name__ == "__main__":
    ...
    if not file_list_for_tui: print("Exiting: No files from report were found in the specified repo path."); exit()

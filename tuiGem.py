...
# ---------------------------------------------------------------------
# 1. CORE PARSING LOGIC (RESTORED TO ORIGINAL)
# ---------------------------------------------------------------------
...
def find_files_in_repo(filenames_to_find, repo_path):
    ...
    # Match the base name from the report filename against the repo map
...
# ---------------------------------------------------------------------
# 3. CORRECTED MAIN APPLICATION LOOP (NON-BLOCKING)
# ---------------------------------------------------------------------
def main(stdscr, file_data):
    ...
    while True:
        ...
        if key == curses.ERR:
            ...
            if key == 17: break
            elif key == curses.KEY_RESIZE: pass # Let the loop handle redraw
            elif key == 19:
                try:
                    ...
                    save_message, save_message_time = f"Successfully saved!", time.time()
                ...
                if state["active_panel"] == "files":
                    ...
                    if key in [curses.KEY_UP, curses.KEY_DOWN]: state["selected_chunk_idx"] = max(0, min(len(active_chunks) - 1, state["selected_chunk_idx"] + (-1 if key == curses.KEY_UP else 1)))
                        ...
                        chunk = active_chunks[state["selected_chunk_idx"]]; editor.insert_chunk(chunk["code"])
        ...
        l_p = stdscr.derwin(screen_height - 1, l_w, 1, 0); c_p = stdscr.derwin(screen_height - 1, c_w, 1, l_w); r_p = stdscr.derwin(screen_height - 1, r_w, 1, l_w + c_w)
        ...
        if save_message and time.time() - save_message_time < 2: stdscr.addstr(0, 0, save_message.ljust(screen_width), curses.color_pair(3))
        else: save_message = ""; stdscr.addstr(0, 0, status.ljust(screen_width), curses.color_pair(6))
...
if __name__ == "__main__":
    ...
    if not file_list_for_tui: print("Exiting: No files from report were found in the search path."); exit()
